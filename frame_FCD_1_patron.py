...
active_thread = None
dist1_var = None
promedio_corriente_fuga_fcd1 = 0.0
promedio_corriente_fcd1 = 0.0

def crear_frame_FCD_1_patron(parent):
    ...
    def calcular_promedios():
        global promedio_corriente_fuga_fcd1
        promedio_carga = sum(cargas) / len(cargas)
        promedio_corriente_fuga_fcd1 = sum(carga / tiempo if tiempo > 0 else 0 for carga, tiempo in zip(cargas, tiempos)) / len(cargas)
        tabla.item("promedio", values=("Promedio", f"{promedio_carga:.5e}", f"{promedio_corriente_fuga_fcd1:.5e}"))
    ...
    def realizar_medidas2():
        ...
        try:
            with serial.Serial('COM8', 9600, timeout=2) as ser:
                ...
                for i in range(5):
                    ...
                    if tiempo_integracion > 0:
                        corriente = (
                            ...
                            - (promedio_corriente_fuga_fcd1 * (101.325 / 293.15) * ((273.15 + temperatura) / presion))
                        )
                    else:
                        corriente = 0.0
                ...
                promedio_carga = sum(cargas) / len(cargas)
                promedio_corriente_fcd1 = sum(carga / tiempo if tiempo > 0 else 0 for carga, tiempo in zip(cargas, tiempos)) / len(cargas)
    ...
    def guardar_datos():
        ...
        datos = {
            "dist1_var": dist1_var.get(),
            "promedio_corriente_fuga_fcd1": promedio_corriente_fuga_fcd1,
            "promedio_corriente_fcd1": promedio_corriente_fcd1,
        }
    ...
    # Frame principal
    fcd1_frame = tk.Frame(parent, bg="white")
    fcd1_frame.pack(fill=tk.BOTH, expand=False, pady=0)

    # Contenedor para la línea de título y campos
    linea_superior = tk.Frame(fcd1_frame, bg="white")
    ...
    titulo = tk.Label(
        ...
        font=("Arial", 16, "bold"),
        bg="light blue"  # Fondo verde limón
    )
    titulo.grid(row=0, column=0, padx=10, pady=15, sticky="w")

    # Campo: Distancia2
    dist1_label = tk.Label(
        linea_superior, 
        text="Distacia cercana (m):", 
        font=("Arial", 16, "bold"),
        bg="light blue"
    )
    dist1_label.grid(row=0, column=2, padx=0, sticky="w")

    dist1_var = tk.StringVar()
    dist1_entry = tk.Entry(
        linea_superior, 
        textvariable=dist1_var,
        font=("Arial", 16, "bold"),
        bg="light blue",
        width=6  # Ancho visual ajustado a 6 caracteres
    )
    dist1_entry.grid(row=0, column=3, padx=5, sticky="w")

    # Campo: Rango del electrómetro
    rango_label = tk.Label(linea_superior, text="Rango del electrómetro:", bg="white")
    rango_label.grid(row=0, column=4, padx=5, sticky="w")

    rango_var = tk.StringVar(value="LOW")
    rango_combo = ttk.Combobox(linea_superior, textvariable=rango_var, state="readonly", width=6)
    rango_combo["values"] = ("LOW", "MEDIUM", "HIGH")
    rango_combo.grid(row=0, column=5, padx=5, sticky="w")
...
##############################
    ...
    tabla_y_botones_frame = tk.Frame(fcd1_frame, bg="white")
    ...
    guardar_btn.grid(row=0, column=4, padx=5, pady=5)  # Colocamos el botón en la columna 4
