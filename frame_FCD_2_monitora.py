...
stop_thread = False
active_thread = None
promedio_corriente_fuga_fcd2_mon = 0.0
promedio_corriente_fcd2_mon = 0.0

def crear_frame_FCD_2_monitora(parent):
    ...
    def calcular_promedios():
        global promedio_corriente_fuga_fcd2_mon
        promedio_carga = sum(cargas) / len(cargas)
        promedio_corriente_fuga_fcd2_mon = sum(carga / tiempo if tiempo > 0 else 0 for carga, tiempo in zip(cargas, tiempos)) / len(cargas)
        tabla.item("promedio", values=("Promedio", f"{promedio_carga:.5e}", f"{promedio_corriente_fuga_fcd2_mon:.5e}"))
    ...
    def realizar_medidas2():
        ...
        try:
            with serial.Serial('COM8', 9600, timeout=2) as ser:
                ...
                for i in range(5):
                    ...
                    if tiempo_integracion > 0:
                        corriente = (
                            ...
                            - (promedio_corriente_fuga_fcd2_mon * (101.325 / 293.15) * ((273.15 + temperatura) / presion))
                        )
                    else:
                        corriente = 0.0
                ...
                promedio_carga = sum(cargas) / len(cargas)
                promedio_corriente_fcd2_mon = sum(carga / tiempo if tiempo > 0 else 0 for carga, tiempo in zip(cargas, tiempos)) / len(cargas)
                ...
                tabla2.item(
                    ...
                    values=(
                        ...
                        f"{promedio_carga:.5e}",
                        f"{promedio_corriente_fcd2_mon:.5e}"
                    )
                )
    ...
    def guardar_datos():
        ...
        datos = {
            "promedio_corriente_fuga_fcd2_mon": promedio_corriente_fuga_fcd2_mon,
            "promedio_corriente_fcd2_mon": promedio_corriente_fcd2_mon,
        }
