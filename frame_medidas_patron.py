...
def crear_frame_medidas_patron(parent):
        ...
        try:
            with serial.Serial('COM8', 9600, timeout=2) as ser:
                ...
                for i in range(5):
                    ...
                    tabla2.item(
                        f"item2_{i}",
                        values=(0, f"{presion:.3f}", f"{temperatura:.3f}", f"{carga:.5e}", f"{corriente:.5e}", f"{kair:.5e}")
                    )
                    parent.update()
    ...
    linea_superior = tk.Frame(frame, bg="white")
    linea_superior.pack(fill=tk.X, pady=20)

    # Título
    titulo = tk.Label(
        linea_superior,
        text="Medidas con la cámara patron",
        font=("Arial", 16, "bold"),
    ...
    vacio_label = tk.Label(linea_superior, text="                          ", bg="white")
    ...
    tabla_y_botones_frame = tk.Frame(frame, bg="white")
    tabla_y_botones_frame.pack(fill=tk.BOTH, expand=False, padx=1, pady=0)

    # Botones a la izquierda de la tabla
    botones_frame = tk.Frame(tabla_y_botones_frame, bg="white")
    botones_frame.grid(row=0, column=0, sticky="nsew", padx=0, pady=50) ##

    iniciar_btn = tk.Button(botones_frame, text="Iniciar", command=iniciar_medicion, bg="light green", fg="black", font=("Arial", 10), width=6)
    iniciar_btn.grid(row=0, column=0, pady=5)

    detener_btn = tk.Button(botones_frame, text="Detener", command=detener_medicion, bg="#FF6666", fg="black", font=("Arial", 10), width=6)
    detener_btn.grid(row=1, column=0, pady=5)

    rehacer_btn = tk.Button(botones_frame, text="Rehacer", command=reiniciar_tabla, bg="light blue", fg="black", font=("Arial", 10), width=6)
    rehacer_btn.grid(row=2, column=0, pady=5)

    # Tabla
    tabla_frame = tk.Frame(tabla_y_botones_frame, bg="white")
    ...
    tabla_frame2 = tk.Frame(tabla_y_botones_frame, bg="white")
    ...
    tabla2 = ttk.Treeview(tabla_frame2, columns=("Tº restante (s)", "Presión (kPa)", "Temp. (ºC)", "Carga (C)", "Corriente (A)", "Kair (Gy/s)"), show="headings", height=6)

    for col in ("Tº restante (s)", "Presión (kPa)", "Temp. (ºC)", "Carga (C)", "Corriente (A)", "Kair (Gy/s)"):
    ...
    botones_frame_derecha = tk.Frame(tabla_y_botones_frame, bg="white")
    botones_frame_derecha.grid(row=0, column=2, sticky="nsew", padx=1, pady=50)

    extra_btn1 = tk.Button(botones_frame_derecha, text="Iniciar", command=iniciar_medicion2, bg="light green", fg="black", font=("Arial", 10), width=6)
    extra_btn1.grid(row=0, column=0, pady=5)

    extra_btn2 = tk.Button(botones_frame_derecha, text="Detener", command=detener_medicion, bg="#FF6666", fg="black", font=("Arial", 10), width=6)
    extra_btn2.grid(row=1, column=0, pady=5)

    extra_btn3 = tk.Button(botones_frame_derecha, text="Rehacer", command=reiniciar_tabla2, bg="light blue", fg="black", font=("Arial", 10), width=6)
    extra_btn3.grid(row=2, column=0, pady=5)

    # Botones adicionales a la derecha de tabla2
    botones_frame_derecha_tabla2 = tk.Frame(tabla_y_botones_frame, bg="white")
    botones_frame_derecha_tabla2.grid(row=0, column=4, sticky="nsew", padx=1, pady=50)

    # Ruta del archivo medidas_patron.json
    json_path_patron = os.path.join("AAA_ST_REGISTROS", "medidas_patron.json")

    # Función para guardar datos en la tabla resumen y archivo JSON
    def guardar_datos_en_tabla_patron():
        global fila_actual2
        ...
        try:
            ...
            medida_key = f"m{fila_actual2 + 1}_patron"
            data = {
                medida_key: {
                    "escala": escala,
            ...
            if os.path.exists(json_path_patron):
                try:
                    with open(json_path_patron, "r") as file:
            ...
            with open(json_path_patron, "w") as file:
    ...
    guardar_btn = tk.Button(
        ...
        command=guardar_datos_en_tabla_patron,
        bg="light green",
        fg="black",
        font=("Arial", 10),
        width=18
    )
    guardar_btn.grid(row=0, column=0, pady=5, padx=2)

    # Botón "Siguiente medida"
    siguiente_btn = tk.Button(
        botones_frame_derecha_tabla2,
        text="Siguiente medida",
        command=siguiente_medida_patron,
        bg="#FFE5CC",
        fg="black",
        font=("Arial", 10),
        width=18
