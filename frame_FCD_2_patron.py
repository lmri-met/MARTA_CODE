...
stop_thread = False
active_thread = None
promedio_corriente_fuga_fcd2 = 0.0
promedio_corriente_fcd2 = 0.0

def crear_frame_FCD_2_patron(parent):
    ...
    def calcular_promedios():
        global promedio_corriente_fuga_fcd2
        promedio_carga = sum(cargas) / len(cargas)
        promedio_corriente_fuga_fcd2 = sum(carga / tiempo if tiempo > 0 else 0 for carga, tiempo in zip(cargas, tiempos)) / len(cargas)
        tabla.item("promedio", values=("Promedio", f"{promedio_carga:.5e}", f"{promedio_corriente_fuga_fcd2:.5e}"))
    ...
    def realizar_medidas2():
        ...
        try:
            with serial.Serial('COM8', 9600, timeout=2) as ser:
                ...
                for i in range(5):
                    ...
                    if tiempo_integracion > 0:
                        corriente = (
                            ...
                            - (promedio_corriente_fuga_fcd2 * (101.325 / 293.15) * ((273.15 + temperatura) / presion))
                        )
                    else:
                        corriente = 0.0
                ...
                promedio_carga = sum(cargas) / len(cargas)
                promedio_corriente_fcd2 = sum(carga / tiempo if tiempo > 0 else 0 for carga, tiempo in zip(cargas, tiempos)) / len(cargas)
    ...
    def guardar_datos():
        ...
        datos = {
            "dist2_var": dist2_var.get(),
            "promedio_corriente_fuga_fcd2": promedio_corriente_fuga_fcd2,
            "promedio_corriente_fcd2": promedio_corriente_fcd2,
        }
    ...
    # Campo: Distancia2
    dist2_label = tk.Label(
        linea_superior, 
        text="Distacia lejana (m):", 
        font=("Arial", 16, "bold"),
        bg="light blue"
    )
    dist2_label.grid(row=0, column=2, padx=0, sticky="w")

    dist2_var = tk.StringVar()
    dist2_entry = tk.Entry(
        linea_superior, 
    ...
    dist2_entry.grid(row=0, column=3, padx=5, sticky="w")
...
##############################
    ...
    guardar_btn3.grid(row=0, column=4, padx=5, pady=5)  # Colocamos el botón en la columna 4
